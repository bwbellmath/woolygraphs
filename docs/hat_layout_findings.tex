...
shaping edges. The signs agree across classes and the effect persists under
further iteration (at 1200 iterations the three are still $+2.1\%$, $+7.9\%$
and $+18.8\%$), so this is the inflate term pulling against the gauge term at
...
coloured by signed relative error against gauge: near-black on gauge, orange
too long, teal too short, saturating at an adjustable full scale. On-gauge
edges recede into the background, so what remains visible is exactly the
problem regions. Beneath the length distributions sits a second plot binning
every edge by that same signed error, with each bar painted in the colour the
heatmap gives that deviation and the full-scale points marked. It doubles as
the heatmap's legend, stating the ramp in numbers beside the distribution it
describes. All three views read the same edge set as the optimizer and share
one colour-ramp function, so they cannot disagree with each other.
